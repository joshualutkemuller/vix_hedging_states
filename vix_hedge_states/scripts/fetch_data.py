...
def main():
    ...
    vvix_df = fetch_ndl('CBOE/VVIX', args.start, args.end, api_key)
    vvix_saved = False
    if vvix_df is not None:
        ...
        if col:
            ...
            vvix_out.to_csv(data_dir / 'vvix.csv', index=False)
            print(f'[OK] Saved {data_dir/"vvix.csv"} ({len(vvix_out)} rows)')
            vvix_saved = True
    if not vvix_saved:
    ...
    for code, colname in series:
        ...
        df = fetch_ndl(code, args.start, args.end, api_key)
        if df is None:
            continue
